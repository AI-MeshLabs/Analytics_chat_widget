...
BEGIN
  ...
  FOREACH col IN ARRAY forbidden_cols LOOP
    IF lowered ~ ('\m' || col || '\M') THEN
      RAISE EXCEPTION 'Column "%" is not allowed. Inbound (calls): call_date, status, duration_secs. Outbound (outbound_call_attempts): call_started_at, outcome, duration_secs.', col;
    END IF;
  END LOOP;
