...
-- Re-run after updates to apply column/table restrictions.
...
BEGIN
  ...
  IF lowered ~ 'call_data' THEN
    RAISE EXCEPTION 'Query must read only from onepoint.calls or onepoint.outbound_call_attempts (call_data is not allowed).';
  END IF;

  IF lowered !~ '(onepoint\.calls|from onepoint\.calls|join onepoint\.calls|from calls|join calls|onepoint\.outbound_call_attempts|from onepoint\.outbound_call_attempts|join onepoint\.outbound_call_attempts|from outbound_call_attempts|join outbound_call_attempts)' THEN
    RAISE EXCEPTION 'Query must read from onepoint.calls and/or onepoint.outbound_call_attempts.';
  END IF;
  ...
  FOREACH col IN ARRAY forbidden_cols LOOP
    IF lowered ~ ('\m' || col || '\M') THEN
      RAISE EXCEPTION 'Column "%" is not allowed. Allowed filter columns include call_date/status/duration_secs/call_started_at/outcome/branch_code/branch_name/cliniko_business_id.', col;
    END IF;
  END LOOP;
